...
w=[0,0]
w1=[0,0]
w1[0]=w[0]=float(input("w1=(帶入值0.1~0.9):"))
w1[1]=w[1]=float(input("w2=(帶入值0.1~0.9):"))
theta=float(input("w1=(帶入值0.1~0.9):"))
a=0.2
xs=[(0,0),(0,1),(1,0),(1,1)]
epouch = 0 #最多做幾個循環
...
def line1(A):
    return (-A*w1[0]+theta)/w1[1] #線性方程    
yb=(input("and 或 or 或 xor"))# vvvvv 判斷 and 或 or
...
while (all_correct < 4 and training_times < 50): #要連續對4次和最多做50個循環
    for i in range(0,4):
        if((xs[i][0]*w1[0]+xs[i][1]*w1[1]-theta)>=0):
            ya=1
        else:
            ya=0
        e=yb[i]-ya
        if e!=0 :
            training_times +=1#調整次數+1
            all_correct = 0 #要連續對4次，所以歸0
            w1[0]=w1[0]+a*xs[i][0]*e
            w1[1]=w1[1]+a*xs[i][1]*e
            print("第{}次".format(training_times),'w3=%4.2f,w4=%4.2f'%(w1[0],w1[1]))
            plt.plot(x,line1(x),"b-")
        else:
            all_correct+=1
print("最後做了第{}次".format(training_times),'w3=%4.2f,w4=%4.2f'%(w1[0],w1[1]))
plt.plot(x,line1(x),"k-")#vvvvvvvv 畫w1和w2的值
plt.show()
